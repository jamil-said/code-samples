...
/* netIncome
You own a small company, and you keep track of its income in the accounting table, which 
...
company during that period. The output should be sorted by the year in ascending order. If there 
are several rows with the same year, sort them by the quarter in ascending order.
...
BEGIN
    SELECT EXTRACT(YEAR FROM date) year, EXTRACT(QUARTER FROM date) quarter, (SUM(profit) - SUM(loss)) net_profit
    from accounting
